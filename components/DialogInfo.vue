<script lang="ts" setup>
  ...
  async function open(item: Record<string, string>) {
    ...
    try {
      const result: EmptyObjectType = await useFetch(`/api/pageinfo`, {
        method: "GET",
        query: { pageName: item.page },
      });
      const response = decrypt(result.data.value.data);
...
<template>
  <v-dialog
    ...
    :max-width="screenMode === 'phone' ? '90%' : '700px'"
    persistent
    scrollable
  >
