<script setup lang="ts">
  ...
  const selectHotArea = (area: Record<number | string, string>) => {
    ...
    if (area.key === "province") {
      state.selectedProvince = area.code;
      emit("select", {
        province: state.selectedProvince,
        city: "",
      });
    } else {
      state.selectedCity = area.code;
      emit("select", {
        province: "",
  ...
  >
    <v-card class="pa-4">
      ...
      <div class="d-flex flex-wrap ga-3 mb-4">
        <v-chip
          v-for="(hot, index) in listHotArea"
          :key="index"
          @click="selectHotArea(hot)"
          :color="state.selectedArea === hot?.code ? 'primary' : undefined"
          class="px-4 py-2"
          rounded="lg"
        >
          {{ hot?.name }}
